...
public enum RouletteLayout {
    ...
    /// The pockets in their PHYSICAL order around the European single-zero wheel,
    /// clockwise starting at zero. This is wheel geometry exactly as the grid below
    /// is felt geometry: stated once, here, and pinned by tests — a visual wheel
    /// drawn from it can never disagree with the resolver about what came up.
    /// (Around the ring, red and black strictly alternate; zero sits between 26
    /// and 32 — properties the tests verify against `redNumbers`.)
    public static let wheelOrder: [Int] = [
